...
from app_018.mz_function import *
...
from .mz_function import encryptpass,key
...
class RemindsViewSet(viewsets.ViewSet):
    ...
    @action(methods=['post'], detail=False)
    @permission_classes([AllowAny])   
    def remindfromtogather(self,request):
       
        def calcreminder():
            reminds=Reminder.objects.all()
            for i in reminds:
                if i.remindtype=='auto':
                    try:
                        i.expiredate=getexpiredate(url=i.url[i.url.index('https://')+8:])
...
class EmailsViewSet(viewsets.ViewSet):
    ...
    def createuser(self,request):
        ...
        createby_password=request.POST.get('password')
        id=request.POST.get('id')
        name=request.POST.get('newfullname')
        username=request.POST.get('newusername')
        password=request.POST.get('newpassword')
        email=request.POST.get('newemail')
        phone=request.POST.get('newphone')
...
#     redirect_ip = "192.168.30.20"

# # ?????? ???? ???? ??????? ?? ????
#     def redirect(pkt):
#     # ???? ??? ???? ?????? ?? ??? HTTP
#         if pkt.haslayer(TCP) and pkt.haslayer(Raw) and b"HTTP" in pkt[Raw].load:
#         # ????? ???? ????? ????
#             ether = Ether(src=pkt[Ether].dst, dst=pkt[Ether].src)
#             ip = IP(src=pkt[IP].dst, dst=pkt[IP].src)
#             tcp = TCP(sport=pkt[TCP].dport, dport=pkt[TCP].sport, flags="FA", seq=pkt[TCP].ack, ack=pkt[TCP].seq + len(pkt[Raw].load))
#         # ????? ??????? ??? ??? IP ??????
#             new_pkt = ether/ip/tcp
#             sendp(new_pkt, iface="ens35")
#             print(f"Redirected HTTP request from {pkt[IP].src} to {redirect_ip}")

# # ??? ???????? ????? ??? ??????
#     sniff(filter="tcp port 80", prn=redirect)
